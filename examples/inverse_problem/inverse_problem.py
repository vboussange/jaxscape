...
with rasterio.open("landcover.tif") as src:
  ...
  raster = src.read(1, masked=True, out_shape=(10, 10), resampling=rasterio.enums.Resampling.mode)  # Read the first band with masking
...
# ax[0].imshow(permeability, cmap="RdYlGn", norm=plt.cm.colors.LogNorm())
# ax[0].set_title('Permeability, first guess')
# ax[0].axis("off")
# fig.colorbar(ax[0].images[0], ax=ax[0], shrink=0.5, location='right')

# dist_to_source_hat = calculate_dist_to_source(permeability)
# ax[1].imshow(dist_to_source_hat, cmap="RdYlGn_r", norm=plt.cm.colors.LogNorm())
# ax[1].set_title('Distance to source, first guess')
# ax[1].axis("off")
# fig.colorbar(ax[1].images[0], ax=ax[1], shrink=0.5, location='right')
# plt.show()

ref_grid = GridGraph(indexed_raster)
source = ref_grid.coord_to_index(jnp.array([0]), jnp.array([0]))
y = true_dist_to_source.ravel()[1:]
x = indexed_raster.ravel()
@nnx.jit  # automatic state management for JAX transforms
def train_step(model, optimizer):
  def loss_fn(model):
    permeability = model(indexed_raster).ravel()
    permeability = jnp.clip(permeability, 1e-2, 1e0)
    permeability = ref_grid.node_values_to_array(permeability)
    grid = GridGraph(permeability)
    dist_to_node_hat = distance(grid, source).ravel()[1:]
    return ((dist_to_node_hat - y) ** 2).mean()
    # return ((permeability - permeability_raster.ravel()) ** 2).mean()
