...
- JAXScape parameterization: the free variable is an unconstrained raster of logits. The benchmark maps logits to permeability with `sigmoid(logits) + 1e-3`, so every cell remains strictly positive. The published profiles compare `CholmodSolver` as the exact CPU baseline plus `AMJaxCGSolver` and the Spielman approximation with dense `pinv` in both `float32` and `float64` where the backend supports it. The AMJax profile intentionally reuses the solver state initialised at the starting iterate throughout optimisation, so degraded convergence is part of the benchmark signal rather than filtered out.
