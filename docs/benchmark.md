...
Formal problem: infer a raster-valued resistance or permeability field parameter $\theta$ by minimizing a discrepancy between observed pairwise dissimilarities $y$ and model-predicted pairwise distances $f_\theta(S)$. The benchmark reports both runtime and the final objective MSE,

$$
\mathrm{MSE} = \frac{1}{n}\sum_i (f_\theta(S)_i - y_i)^2.
$$

Synthetic setup: the inverse benchmark now uses one deterministic 100x100 raster with 20 seeded sample sites, then derives the target pairwise response from that same synthetic landscape. The optimisation task is therefore a controlled regression problem rather than an empirical data-fitting exercise.

Software entry points:

- JAXScape: `optx.minimise(...)` with `optx.LBFGS`, minimizing the mean-squared error between `ResistanceDistance(...)(GridGraph(sigmoid(logits) + 1e-3, fun=edge_weight), nodes=sample_coords)` and an exact synthetic target matrix generated once with `CholmodSolver`.
...
- JAXScape parameterization: the free variable is an unconstrained raster of logits. The benchmark maps logits to permeability with `sigmoid(logits) + 1e-3`, so every cell remains strictly positive. The published profiles compare `CholmodSolver` as the exact CPU baseline plus `AMJaxCGSolver` and the Spielman approximation with dense `pinv` in both `float32` and `float64` where the backend supports it. The AMJax profile intentionally reuses the preconditioner initialised at the starting iterate while refreshing CG state for the current Laplacian throughout optimisation, so degraded preconditioner quality is part of the benchmark signal rather than filtered out.
...
Fairness notes: both optimisation paths are intentionally budget-limited. The chart marks converged inverse runs with filled markers and iteration-budget hits with hollow markers, so runtime and final-fit quality can be read together.
