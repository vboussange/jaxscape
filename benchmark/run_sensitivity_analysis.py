...
def compute_connectivity_in_window(hab_qual, valid_activities, D, distance, cut_off):
...
grad_connectivity_in_window = grad(compute_connectivity_in_window)
...
def run_sensitivity_analysis(habitat_quality_raster, window_op, D, distance, cut_off):
    ...
    for x_start, y_start, hab_qual in tqdm(window_op.iterate_windows(habitat_quality_raster), total=window_op.nb_steps, desc="Running sensitivity analysis"):
        ...
        sensitivity_raster_window = grad_connectivity_in_window(hab_qual, valid_activities, D, distance, cut_off)
