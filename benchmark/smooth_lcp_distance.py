"""
Illustrating the use of the SmoothLCPDistance class to calculate the distance to a focal pixel in a habitat raster.
"""
import jax
...
path_results = Path("results/SmoothLCPDistance")
...
distance = SmoothLCPDistance(tau=1e-10)
vertex_index = grid.coord_to_active_vertex_index(*focal_pixel_coord)
distance_to_node = grid.node_values_to_array(distance(grid, jnp.array([vertex_index])).flatten())
fig, ax = plt.subplots()
...
fig.suptitle("SmoothLCPDistance")
fig.tight_layout()
